...
export default function LoginStubScreen() {
  ...
  const onSubmit = async (channel: "wcall" | "telegram" = "wcall") => {
    ...
    setBusy(true);
    try {
      const data = await apiFetch(channel === "telegram" ? "/auth/specialist/phone/send-otp" : "/auth/specialist/login", {
        method: "POST",
        body: JSON.stringify({ phone: apiPhone, password, role: "specialist", purpose: "login", channel }),
        auth: false,
      });
      if (data?.otp_id) {
  ...
  const verify = async () => {
    ...
    setBusy(true);
    try {
      const data = await apiFetch("/auth/specialist/phone/verify-otp", { method: "POST", body: JSON.stringify({ phone: apiPhone, otp_id: otpId, code: otpChannel === "wcall" ? "" : otpCode.trim() }), auth: false });
      await signIn(data.token, data.user);
