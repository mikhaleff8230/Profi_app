...
export default function ChatScreen() {
  ...
  const customerAvatar = fileUrl(chat?.customer_avatar);
  const customerName = chat?.customer_name || "Клиент";
  const list = messages ?? [];
  ...
  return (
    <SafeAreaView style={styles.root} edges={["top", "left", "right"]}>
      <View style={styles.chatHeader}>
        ...
        <TouchableOpacity style={styles.customerHead} onPress={() => navigation.navigate("CustomerProfile", { chatId: String(chatId) })} activeOpacity={0.75}>
          {customerAvatar ? <Image source={{ uri: customerAvatar }} style={styles.headerAvatar} /> : <View style={[styles.headerAvatar, styles.headerFallback]}><Text style={styles.headerLetter}>{customerName.charAt(0).toUpperCase()}</Text></View>}
          <View style={styles.headerTexts}>
            <Text style={styles.customerName} numberOfLines={1}>{customerName}</Text>
            <Text style={styles.taskTitle} numberOfLines={2}>{chat?.task_title || chat?.title || t("chats")}</Text>
          </View>
        </TouchableOpacity>
        <TouchableOpacity style={styles.headerMore} onPress={() => navigation.navigate("CustomerProfile", { chatId: String(chatId) })}><Ionicons name="ellipsis-horizontal" size={22} color={colors.black} /></TouchableOpacity>
      </View>
...
const styles = StyleSheet.create({
  ...
  customerHead: { flex: 1, minWidth: 0, flexDirection: "row", alignItems: "center", gap: 10 },
  ...
  headerTexts: { flex: 1, minWidth: 0 }, customerName: { fontSize: 15, fontWeight: "900", color: colors.black }, taskTitle: { marginTop: 2, fontSize: 12, lineHeight: 16, color: colors.neutral500 },
  headerMore: { width: 38, height: 42, alignItems: "center", justifyContent: "center" },
  flex: { flex: 1 },
